...
test.describe("Dashboard-kaders slepen", () => {
  test("standaardvolgorde toont alle kaders behalve het (verborgen) voorstellen-kader", async ({ page }) => {
    await gaNaarGastDashboard(page);
    const volgorde = await huidigeVolgorde(page);
    expect(volgorde).toEqual(["samenvatting", "grafieken", "inkomen", "kosten", "wat-als", "doelen-investeringen"]);
  });
